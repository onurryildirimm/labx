    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="acceptForm">
          ...
          <!-- Gizli alanlar (Güncelle modalında görünür) -->
...
$(function(){
    ...
    $('.update-btn').on('click', function(){
        ...
        $('#f_reason').val(row.reason || 'OZEL_ISTEK');
        
        var types = [];
        try { types = JSON.parse(row.types_json || '[]'); } catch(e){}
        $('#f_type').val(types[0] || 0);
        filterSubtypes();
    ...
    $('#saveAccept').on('click', function(){
        ...
        $.post('sample-accept-new.php?lang=<?=$lang?>&db=<?=$selectedDb?>', data, function(res){
            if(res && res.success){
                location.reload();
            } else {
    ...
    $('.delete-btn').on('click', function(){
        ...
        $.post('sample-accept-new.php?lang=<?=$lang?>&db=<?=$selectedDb?>', 
